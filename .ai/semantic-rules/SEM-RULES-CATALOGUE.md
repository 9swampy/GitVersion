...
SEM-RULE-NNN:
  ...
  Severity:   Error | Warning | Advisory  (Advisory is the post-DEC-016 default for valid-but-fragile configs)
```
...
Severity: Advisory (per DEC-016 ARC-001 — labelled CD is valid; remediation is optional)
```
...
Severity: Advisory (per DEC-016 ARC-001 — implicit defaults are valid; declaring is optional hardening)
```
...
Severity: Advisory (per DEC-016 ARC-001 — engine-default inheritance is valid; explicit declaration is optional upgrade-hardening)
```
