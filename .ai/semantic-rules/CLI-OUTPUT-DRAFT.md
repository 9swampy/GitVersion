...
❌  SEM-004  Warning  branch 'develop'
    Deployment mode conflict.
    Root mode is ContinuousDeployment but branch 'develop' carries label 'alpha'
    with no explicit mode override. ContinuousDeployment will suppress the
    prerelease label — every commit on develop will appear as a clean release.

    Remediation: Add mode: ContinuousDelivery to this branch, or change
    the root mode to ContinuousDelivery.

⚠   SEM-006  Warning  (root)
    No strategies block declared.
    GitVersion's default strategy set has changed across versions. Without
    an explicit list, behaviour may change on upgrade.
