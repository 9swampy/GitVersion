    ...
    : IGitVersionExecutor
{
    ...
    private int RunValidation(GitVersionOptions gitVersionOptions)
    {
        ...
        {
            EmitJson(violations, errors, warnings);
        }
        else
        {
            EmitText(violations, errors, warnings);
        }

        return errors > 0 ? 1 : 0;
    }

    private void EmitText(IReadOnlyList<SemanticViolation> violations, int errors, int warnings)
    {
        this.console.WriteLine($"GitVersion Semantic Configuration Validator{FileSystemHelper.Path.NewLine}");
        ...
        if (errors == 0 && warnings == 0)
        {
            ...
            this.console.WriteLine($"{FileSystemHelper.Path.NewLine}  0 errors · 0 warnings");
        ...
        else
        {
            ...
            this.console.WriteLine(errors > 0
                ...
                : "  Configuration is semantically valid (warnings noted above).");
        }
    }

    private void EmitJson(IReadOnlyList<SemanticViolation> violations, int errors, int warnings)
    {
        var result = new
        {
            valid = errors == 0,
            summary = new { errors, warnings },
